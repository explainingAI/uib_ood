...
__all__ = ['get_random_images']


def __generate_salt_and_pepper(size: tuple, p=0.5):
    ...
    out[np.random.rand(*size) < p] = 1
    return out


def __generate_gaussian_noise(size: tuple, mean=0, std=1):
...
def __generate_speckle_noise(size: tuple, mean=0, std=1):
...
def __generate_constant_images(size, value: Union[int, float]):
...
def __generate_random_contant_images(size: tuple):
    """ Generate constant images with random values.

    Args:
        size (Tuple): Size of the image.
    """
    return __generate_constant_images(size, np.random.uniform(0, 255) / 255)
...
def get_random_images(n_images: int, size: tuple):
    ...
    for i in range(n_images // 4):
        ...
        images_funcs = [__generate_random_contant_images, __generate_gaussian_noise,
                        __generate_speckle_noise, __generate_salt_and_pepper]
